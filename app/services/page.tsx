...
// ─── Data ─────────────────────────────────────────────────────────────────────
const ROLES = [
  "Hostess", "VIP Hostess", "Usher", "Greeter", "Receptionist",
  "Registration Staff", "Promoter", "Sales Promoter", "Brand Ambassador",
  "Sampling Staff", "Lead Generator", "Model", "Promotional Model",
  "Fashion Model", "Exhibition Staff", "Event Support Staff",
  "Hospitality Staff", "Customer Service Representative",
  "Temporary Corporate Staff", "Stand Speaker", "MC / Anchor",
  "Event Coordinator", "Team Leader", "Supervisor", "Security Staff",
];
...
export default function Services() {
  return (
    <div className="bg-[var(--background)] min-h-screen">
      ...
      {/* ── Roles we provide ── */}
      <section className="py-16 md:py-20 bg-[var(--background)] border-t border-[var(--border)]">
        ...
        <div className="container mx-auto px-5 max-w-5xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {INDUSTRIES.map((ind, i) => (
              ...
              >
                <div className="w-11 h-11 rounded-xl bg-[image:var(--gradient-primary)] flex items-center justify-center text-white text-base mb-4 group-hover:scale-110 transition-transform shadow-[var(--shadow-sm)]">
                  {ind.icon}
                </div>
                <h3 className="font-bold text-sm text-[var(--text-primary)] mb-1">{ind.title}</h3>
                <p className="text-xs text-[var(--text-secondary)] leading-relaxed">{ind.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          >
            {OCCASIONS.map((occ, i) => (
              <motion.div
                ...
                whileHover={{ scale: 1.04, y: -2 }}
                className="px-4 py-2 rounded-full bg-[var(--primary-muted)] border border-[var(--border)] hover:border-[var(--primary)]/40 cursor-default transition-all"
              >
                <span className="text-[var(--primary)] text-sm font-semibold">{occ}</span>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ── How it works ── */}
      <section className="py-16 md:py-20 bg-[var(--surface-elevated)] border-t border-[var(--border)] relative overflow-hidden">
          ...
          <div className="max-w-3xl mx-auto space-y-2.5">
            {PROCESS.map((item, i) => (
              <motion.div
                ...
                viewport={{ once: true }} transition={{ delay: i * 0.06 }}
                className="glass-card px-5 py-4 rounded-sm flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-5 hover:border-[var(--primary)]/25 border border-transparent transition-all"
              >
                <span className="shrink-0 sm:w-28 text-[10px] font-black uppercase tracking-widest text-[var(--primary)]">{item.when}</span>
                <span className="shrink-0 sm:w-44 font-bold text-sm text-[var(--text-primary)]">{item.title}</span>
